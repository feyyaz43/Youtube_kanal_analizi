 ...
 * @author PC
 */
public class Ozel_Video_islem4 implements Runnable {

    LinkedBlockingQueue<String> islem4_kuyruk;
...
//    QueryRunner qr = new QueryRunner(ds);
    ...
    public Ozel_Video_islem4(LinkedBlockingQueue<String> islem4_kuyruk, QueryRunner qr) {
...
//            PreparedStatement preparedStatement = null;
            ...
            try {
                video_idsi = islem4_kuyruk.take();
            } catch (InterruptedException ex) {
                Logger.getLogger(Ozel_Video_islem4.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            try {
                ...
                String sql_ozel_video_islem4 = "insert into samet_feyyaz.ozel_video_depo"
...
//                try {
//                    preparedStatement = ds.getConnection().prepareStatement(sql_ozel_video_islem4);
...
//                Logger.getLogger(Ozel_Video_islem4.class.getName()).log(Level.SEVERE, null, ex);
                try {
                    ...
                    Logger.getLogger(Ozel_Video_islem4.class.getName()).log(Level.SEVERE, null, ex1);
                }
            }
...
class Ozel_Video_islem4_Kuyruk_Getir implements Runnable {
...
//    ArrayListHandler alh = new ArrayListHandler();
    public Ozel_Video_islem4_Kuyruk_Getir(LinkedBlockingQueue<String> islem4_kuyruk, QueryRunner qr) {
        this.islem4_kuyruk = islem4_kuyruk;
        this.qr = qr;
    }
    ...
    public void run() {
        while (true) {
            ...
            try {
                query = qr.query("select video_id from samet_feyyaz.ozel_video where 36<=sorgu_sayisi and sorgu_sayisi<48", alh);
                qr.update("update samet_feyyaz.ozel_video set sorgu_sayisi = sorgu_sayisi+1  where 36<=sorgu_sayisi and sorgu_sayisi<48");
            } catch (SQLException ex) {
                Logger.getLogger(Ozel_Video_islem4_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
            }

            for (Object[] object : query) {
                if (object != null) {
                    try {
                        islem4_kuyruk.put(object[0].toString());
                    } catch (InterruptedException ex) {
                        Logger.getLogger(Ozel_Video_islem4_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            ...
            try {
                Thread.sleep(14 * 60 * 60 * 1000);       //14 saatte bir video kontrol
            } catch (InterruptedException ex) {
                Logger.getLogger(Ozel_Video_islem4_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
