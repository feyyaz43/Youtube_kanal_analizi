 ...
 * @author PC
 */
public class Ozel_Video_islem5 implements Runnable {

    LinkedBlockingQueue<String> islem5_kuyruk;
...
//    PreparedStatement preparedStatement = null;
    public Ozel_Video_islem5(LinkedBlockingQueue<String> islem5_kuyruk, QueryRunner qr) {
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                video_idsi = islem5_kuyruk.take();
            } catch (InterruptedException ex) {
                Logger.getLogger(Ozel_Video_islem5.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            try {
                ...
                String sql_ozel_video_islem5 = "insert into samet_feyyaz.ozel_video_depo"
                        ...
                        + "values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
//                try {
//                    preparedStatement = ds.getConnection().prepareStatement(sql_ozel_video_islem5);
...
//                Logger.getLogger(Ozel_Video_islem5.class.getName()).log(Level.SEVERE, null, ex);
                try {
                    ...
                    Logger.getLogger(Ozel_Video_islem5.class.getName()).log(Level.SEVERE, null, ex1);
                }
            }
...
class Ozel_Video_islem5_Kuyruk_Getir implements Runnable {
...
//    ArrayListHandler alh = new ArrayListHandler();
    public Ozel_Video_islem5_Kuyruk_Getir(LinkedBlockingQueue<String> islem5_kuyruk, QueryRunner qr) {
        this.islem5_kuyruk = islem5_kuyruk;
        this.qr = qr;
    }
    ...
    public void run() {
        while (true) {
            ...
            try {
                query = qr.query("select video_id from samet_feyyaz.ozel_video where 48<=sorgu_sayisi and sorgu_sayisi<78", alh);
                qr.update("update samet_feyyaz.ozel_video set sorgu_sayisi = sorgu_sayisi+1  where 48<=sorgu_sayisi and sorgu_sayisi<78");
            } catch (SQLException ex) {
                Logger.getLogger(Ozel_Video_islem5_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
            }

            for (Object[] object : query) {
                if (object != null) {
                    try {
                        islem5_kuyruk.put(object[0].toString());
                    } catch (InterruptedException ex) {
                        Logger.getLogger(Ozel_Video_islem5_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            ...
            try {
                Thread.sleep(24 * 60 * 60 * 1000);       //24 saatte bir video kontrol
            } catch (InterruptedException ex) {
                Logger.getLogger(Ozel_Video_islem5_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
