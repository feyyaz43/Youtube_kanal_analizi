 ...
 * @author PC
 */
public class Ozel_Video_islem3 implements Runnable {

    LinkedBlockingQueue<String> islem3_kuyruk;
...
//    QueryRunner qr = new QueryRunner(ds);
    ...
    public Ozel_Video_islem3(LinkedBlockingQueue<String> islem3_kuyruk, QueryRunner qr) {
...
//            PreparedStatement preparedStatement = null;
            ...
            try {
                video_idsi = islem3_kuyruk.take();
            } catch (InterruptedException ex) {
                Logger.getLogger(Ozel_Video_islem3.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            try {
                ...
                String sql_ozel_video_islem3 = "insert into samet_feyyaz.ozel_video_depo"
...
//                try {
//                    preparedStatement = ds.getConnection().prepareStatement(sql_ozel_video_islem3);
...
//                Logger.getLogger(Ozel_Video_islem3.class.getName()).log(Level.SEVERE, null, ex);
                try {
                    ...
                    Logger.getLogger(Ozel_Video_islem3.class.getName()).log(Level.SEVERE, null, ex1);
                }
...
class Ozel_Video_islem3_Kuyruk_Getir implements Runnable {
...
//    ArrayListHandler alh = new ArrayListHandler();
    public Ozel_Video_islem3_Kuyruk_Getir(LinkedBlockingQueue<String> islem3_kuyruk, QueryRunner qr) {
        this.islem3_kuyruk = islem3_kuyruk;
        this.qr = qr;
    }
    ...
    public void run() {
        while (true) {
            ...
            try {
                query = qr.query("select video_id from samet_feyyaz.ozel_video where 24<=sorgu_sayisi and sorgu_sayisi<36", alh);
                qr.update("update samet_feyyaz.ozel_video set sorgu_sayisi = sorgu_sayisi+1  where 24<=sorgu_sayisi and sorgu_sayisi<36");
            } catch (SQLException ex) {
                Logger.getLogger(Ozel_Video_islem3_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
            }

            for (Object[] object : query) {
                if (object != null) {
                    try {
                        islem3_kuyruk.put(object[0].toString());
                    } catch (InterruptedException ex) {
                        Logger.getLogger(Ozel_Video_islem3_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            ...
            try {
                Thread.sleep(8 * 60 * 60 * 1000);       //8 saatte bir video kontrol
            } catch (InterruptedException ex) {
                Logger.getLogger(Ozel_Video_islem3_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
