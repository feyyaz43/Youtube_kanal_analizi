 ...
 * @author PC
 */
public class Ozel_Video_islem8 implements Runnable {

    LinkedBlockingQueue<String> islem8_kuyruk;
...
//    QueryRunner qr = new QueryRunner(ds);
    ...
    public Ozel_Video_islem8(LinkedBlockingQueue<String> islem8_kuyruk, QueryRunner qr) {
...
//            PreparedStatement preparedStatement = null;
            ...
            try {
                video_idsi = islem8_kuyruk.take();
            } catch (InterruptedException ex) {
                Logger.getLogger(Ozel_Video_islem8.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            try {
                ...
                String sql_ozel_video_islem8 = "insert into samet_feyyaz.ozel_video_depo"
...
//                try {
//                    preparedStatement = ds.getConnection().prepareStatement(sql_ozel_video_islem8);
...
//                Logger.getLogger(Ozel_Video_islem8.class.getName()).log(Level.SEVERE, null, ex);
                try {
                    ...
                    Logger.getLogger(Ozel_Video_islem8.class.getName()).log(Level.SEVERE, null, ex1);
                }
            }
...
class Ozel_Video_islem8_Kuyruk_Getir implements Runnable {
...
//    ArrayListHandler alh = new ArrayListHandler();
    public Ozel_Video_islem8_Kuyruk_Getir(LinkedBlockingQueue<String> islem8_kuyruk, QueryRunner qr) {
        this.islem8_kuyruk = islem8_kuyruk;
        this.qr = qr;
    }
    ...
    public void run() {
        while (true) {
            ...
            try {
                query = qr.query("select video_id from samet_feyyaz.ozel_video where 105<=sorgu_sayisi", alh);
                qr.update("update samet_feyyaz.ozel_video set sorgu_sayisi = sorgu_sayisi+1  where 105<=sorgu_sayisi");
            } catch (SQLException ex) {
                Logger.getLogger(Ozel_Video_islem8_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
            }

            for (Object[] object : query) {
                if (object != null) {
                    try {
                        islem8_kuyruk.put(object[0].toString());
                    } catch (InterruptedException ex) {
                        Logger.getLogger(Ozel_Video_islem8_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            ...
            try {
                Thread.sleep(240 * 60 * 60 * 1000);       //720 saatte bir video kontrol
            } catch (InterruptedException ex) {
                Logger.getLogger(Ozel_Video_islem8_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
            }
            try {
