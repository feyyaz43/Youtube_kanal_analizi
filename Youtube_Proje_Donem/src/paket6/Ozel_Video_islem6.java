 ...
 * @author PC
 */
public class Ozel_Video_islem6 implements Runnable {

    LinkedBlockingQueue<String> islem6_kuyruk;
...
//    QueryRunner qr = new QueryRunner(ds);
    ...
    public Ozel_Video_islem6(LinkedBlockingQueue<String> islem6_kuyruk, QueryRunner qr) {
...
//            PreparedStatement preparedStatement = null;
            ...
            try {
                video_idsi = islem6_kuyruk.take();
            } catch (InterruptedException ex) {
                Logger.getLogger(Ozel_Video_islem6.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            try {
                ...
                String sql_ozel_video_islem6 = "insert into samet_feyyaz.ozel_video_depo"
...
//                try {
//                    preparedStatement = ds.getConnection().prepareStatement(sql_ozel_video_islem6);
...
//                Logger.getLogger(Ozel_Video_islem6.class.getName()).log(Level.SEVERE, null, ex);
                try {
                    ...
                    Logger.getLogger(Ozel_Video_islem6.class.getName()).log(Level.SEVERE, null, ex1);
                }
            }
...
class Ozel_Video_islem6_Kuyruk_Getir implements Runnable {
...
//    ArrayListHandler alh = new ArrayListHandler();
    public Ozel_Video_islem6_Kuyruk_Getir(LinkedBlockingQueue<String> islem6_kuyruk, QueryRunner qr) {
        this.islem6_kuyruk = islem6_kuyruk;
        this.qr = qr;
    }
    ...
    public void run() {
        while (true) {
            ...
            try {
                query = qr.query("select video_id from samet_feyyaz.ozel_video where 78<=sorgu_sayisi and sorgu_sayisi<87", alh);
                qr.update("update samet_feyyaz.ozel_video set sorgu_sayisi = sorgu_sayisi+1  where 78<=sorgu_sayisi and sorgu_sayisi<87");
            } catch (SQLException ex) {
                Logger.getLogger(Ozel_Video_islem6_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
            }

            for (Object[] object : query) {
                if (object != null) {
                    try {
                        islem6_kuyruk.put(object[0].toString());
                    } catch (InterruptedException ex) {
                        Logger.getLogger(Ozel_Video_islem6_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            ...
            try {
                Thread.sleep(72 * 60 * 60 * 1000);       //72 saatte bir video kontrol
            } catch (InterruptedException ex) {
                Logger.getLogger(Ozel_Video_islem6_Kuyruk_Getir.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
